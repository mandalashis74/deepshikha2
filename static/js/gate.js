...
async function _loadData() {
    ...
    // Fetch owners (flats list for directory)
    try {
        const { data } = await sbClient.from('owners').select('flat_no, owner_name').order('flat_no');
        if (data) _owners = data;
    } catch { _owners = []; }
}
...
async function _renderGuardDashboard(container) {
    ...
    const staffToday = _monthlyStaff.filter(s => s.is_active);
    if (staffToday.length === 0) {
        html += '<div style="text-align:center;padding:16px;color:var(--text-muted);">No registered staff</div>';
    } else {
        html += '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px;">';
        for (const s of staffToday) {
            // Check if currently checked in
            const checkedIn = _gatePasses.filter(p => p.monthly_staff_id === s.id && p.status === 'checked_in').length > 0;
            ...
            else html += '<div style="width:36px;height:36px;border-radius:50%;background:var(--color-indigo);color:#fff;display:flex;align-items:center;justify-content:center;font-size:0.9rem;">' + (s.name.charAt(0).toUpperCase()) + '</div>';
            html += '<div style="flex:1;"><strong style="font-size:0.8rem;">' + escapeHtml(s.name) + '</strong><br><span style="font-size:0.65rem;color:var(--text-muted);">' + escapeHtml(s.flat_no) + ' · ' + escapeHtml(s.purpose || '') + '</span></div>';
            if (checkedIn) {
                html += '<button class="btn btn-sm" style="background:var(--color-rose);color:#fff;font-size:0.65rem;padding:2px 8px;" onclick="gateStaffCheckOut(\'' + s.id + '\')"><i class="fa-solid fa-right-from-bracket"></i> Out</button>';
            } else {
                html += '<button class="btn btn-sm" style="background:var(--color-emerald);color:#fff;font-size:0.65rem;padding:2px 8px;" onclick="gateStaffCheckIn(\'' + s.id + '\')"><i class="fa-solid fa-right-to-bracket"></i> In</button>';
            }
            html += '</div>';
        }
        html += '</div>';
...
async function _renderMonthlyStaff(container) {
    container.innerHTML = '<div style="text-align:center;padding:40px;"><i class="fa-solid fa-spinner fa-spin"></i></div>';
    const flats = _owners.length > 0 ? _owners : [];
    // If no owners loaded, fall back to flats from staff records
    const fallbackFlats = [...new Set(_monthlyStaff.map(s => s.flat_no))].sort();
    const flatList = flats.length > 0 ? flats : fallbackFlats.map(f => ({ flat_no: f, owner_name: '' }));

    let html = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">';
    html += '<h3 style="margin:0;font-size:0.95rem;"><i class="fa-solid fa-users"></i> Staff Directory by Flat</h3>';
    html += '<span style="font-size:0.8rem;color:var(--text-muted);">' + flatList.length + ' flats · ' + _monthlyStaff.length + ' staff</span>';
    html += '</div>';

    let anyStaff = false;
    for (const flat of flatList) {
        const flatStaff = _monthlyStaff.filter(s => s.flat_no === flat.flat_no);
        if (flatStaff.length > 0) anyStaff = true;
        ...
        html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:var(--bg-body);border-bottom:1px solid var(--border-color);cursor:pointer;" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display===\'none\'?\'block\':\'none\'">';
        ...
        html += ' <span style="font-size:0.7rem;color:var(--text-muted);">· ' + flatStaff.length + ' staff</span>';
        html += '</div>';
        html += '<div><i class="fa-solid fa-chevron-down" style="color:var(--text-muted);font-size:0.75rem;"></i></div></div>';
        // Staff body
        html += '<div style="display:' + (flatStaff.length > 0 || _hasGate('gate:view') ? 'block' : 'none') + ';">';
        if (flatStaff.length === 0) {
            html += '<div style="padding:16px;text-align:center;color:var(--text-muted);font-size:0.8rem;">No staff registered for this flat</div>';
        } else {
            for (const s of flatStaff) {
                const todayPass = _gatePasses.filter(p => p.monthly_staff_id === s.id && p.status === 'checked_in');
                const isCheckedIn = todayPass.length > 0;
                const lastPass = _gatePasses.filter(p => p.monthly_staff_id === s.id).sort((a,b) => new Date(b.created_at) - new Date(a.created_at))[0];
                ...
                else html += '<div style="width:40px;height:40px;border-radius:50%;background:var(--color-indigo);color:#fff;display:flex;align-items:center;justify-content:center;font-size:0.9rem;flex-shrink:0;">' + (s.name.charAt(0).toUpperCase()) + '</div>';
                ...
                if (s.phone) html += '<span style="font-size:0.7rem;color:var(--text-muted);"><i class="fa-solid fa-phone"></i> ' + escapeHtml(s.phone) + '</span>';
                html += '</div>';
                if (isCheckedIn) html += '<span style="font-size:0.7rem;color:var(--color-emerald);"><i class="fa-solid fa-circle"></i> On premises</span>';
                else if (lastPass) html += '<span style="font-size:0.7rem;color:var(--text-muted);">Last visit: ' + _timeAgo(lastPass.created_at) + '</span>';
                else html += '<span style="font-size:0.7rem;color:var(--text-muted);">No visits yet</span>';
                html += '</div>';
                html += '<div style="display:flex;gap:4px;flex-shrink:0;">';
                if (_isGuard()) {
                    if (isCheckedIn) {
                        html += '<button class="btn btn-sm" style="background:var(--color-rose);color:#fff;font-size:0.6rem;padding:2px 6px;" onclick="gateStaffCheckOut(\'' + s.id + '\')"><i class="fa-solid fa-right-from-bracket"></i></button>';
                    } else {
                        html += '<button class="btn btn-sm" style="background:var(--color-emerald);color:#fff;font-size:0.6rem;padding:2px 6px;" onclick="gateStaffCheckIn(\'' + s.id + '\')"><i class="fa-solid fa-right-to-bracket"></i></button>';
                    }
                }
                if (_hasGate('gate:view')) {
                    html += '<button class="btn btn-sm" style="font-size:0.6rem;padding:2px 6px;" onclick="gateOpenEditStaff(\'' + s.id + '\')"><i class="fa-solid fa-pen"></i></button>';
                }
                html += '</div></div>';
            }
        }
        // Add staff button for this flat
        if (_hasGate('gate:view')) {
            html += '<div style="padding:8px 14px;text-align:center;border-top:1px dashed var(--border-color);">';
            html += '<button class="btn btn-sm" style="font-size:0.7rem;padding:2px 12px;" onclick="gateOpenAddStaff(\'' + escapeHtml(flat.flat_no) + '\')"><i class="fa-solid fa-plus"></i> Add Staff</button>';
            html += '</div>';
        }
        html += '</div></div>';
    }
    if (!anyStaff && flatList.length === 0) {
        html = '<div style="text-align:center;padding:40px;color:var(--text-muted);"><i class="fa-solid fa-user-plus" style="font-size:2rem;"></i><br><br>No flats or staff found.<br>Import owners first in Administration.</div>';
    }
    container.innerHTML = html;
}
...
window.gateStaffCheckIn = async function(staffId) {
    const staff = _monthlyStaff.find(s => s.id === staffId);
    if (!staff) return;
    try {
        const insData = {
            visitor_name: staff.name,
            flat_no: staff.flat_no,
            purpose: staff.purpose || 'Monthly Staff',
            pass_type: 'monthly_pass',
            status: 'checked_in',
            monthly_staff_id: staffId,
            checked_in_at: new Date().toISOString(),
            created_by: _userId()
        };
        await sbClient.from('visitor_passes').insert(insData);
        await sbClient.from('gate_log').insert({
            staff_id: staffId, action: 'check_in', performed_by: _userId(),
            flat_no: staff.flat_no, visitor_name: staff.name, remarks: ''
        }).maybeSingle();
        showToast(staff.name + ' checked in ✓', 'success');
    } catch (err) { showToast('Error: ' + err.message, 'error'); }
};

// --- Guard: Staff Check Out ---
window.gateStaffCheckOut = async function(staffId) {
    try {
        const now = new Date().toISOString();
        const { data: active } = await sbClient.from('visitor_passes')
            .select('id').eq('monthly_staff_id', staffId).eq('status', 'checked_in').limit(1);
        ...
        const staff = _monthlyStaff.find(s => s.id === staffId);
        await sbClient.from('gate_log').insert({
            staff_id: staffId, action: 'check_out', performed_by: _userId(),
            flat_no: staff ? staff.flat_no : '', visitor_name: staff ? staff.name : '', remarks: ''
        }).maybeSingle();
        showToast('Checked out ✓', 'success');
    } catch (err) { showToast('Error: ' + err.message, 'error'); }
};
